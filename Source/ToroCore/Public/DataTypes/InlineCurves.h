...
struct TOROCORE_API FInlineFloatCurve final
{
	...
	FORCEINLINE bool HasAnyData() const { return GetRichCurveConst()->HasAnyData(); }

	FRichCurve* GetRichCurve() { return Curve.GetRichCurve(); }
	const FRichCurve* GetRichCurveConst() const { return Curve.GetRichCurveConst(); }
...
struct TOROCORE_API FInlineVectorCurve final
{
	...
	FRichCurve* GetRichCurve(int32 Index) { return Curve.GetRichCurve(Index); }
	const FRichCurve* GetRichCurveConst(int32 Index) const { return Curve.GetRichCurveConst(Index); }

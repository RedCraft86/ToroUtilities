...
struct TOROCORE_API FWireShapeBase
{
    ...
    FWireShapeBase(): Color(FColor::Cyan), bRenderOnTop(false) {}
};
...
struct TOROCORE_API FWireNavPathData final : public FWireShapeBase
{
    ...
    void FindNearestPoint(const FVector& CamVector)
    {
        ...
        float LastDistSq = FVector::DistSquared(PathPoints[0], CamVector);
        for (int32 i = 1; i < PathPoints.Num(); i++)
        {
            const float DistSq = FVector::DistSquared(PathPoints[i], CamVector);

...
{
	const TSharedPtr<IDetailLayoutBuilder> DetailBuilder = WeakBuilder.Pin();
	check(DetailBuilder.IsValid());

	if (const FToroCategoryInfo* Info = CategoryMap.Find(Category))
	{
		return DetailBuilder->EditCategory(Category, Info->DisplayName, Info->Priority);
	}

	return DetailBuilder->EditCategory(Category);
}
...
#pragma region ClassCategory
	...
	bool bHideAllNativeCategories = ClassHideCategories.Remove(FString(TEXT("*"))) > 0;
	...
	{
		// Renamed categories are part of the show-only allowlist.
		AllowedCategories.Add(Rename.Key);
	}
	...
	DetailBuilder->GetCategoryNames(HidingCategories);
...
void FToroStructCustomization::ForwardMetadata(const TSharedPtr<IPropertyHandle>& Property) const
{
	...
	if (StructHandle.IsValid() && Property.IsValid())
	{

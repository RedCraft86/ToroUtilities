...
public:
	...
	static void SetOffsetX(const float InValue)
	{
		OffsetX = FMath::Max(FMath::RoundToInt(InValue), 1);
	}

	static inline int32 OffsetY = 500;
	static float GetOffsetY() { return OffsetY; }
	static void SetOffsetY(const float InValue)
	{
		OffsetY = FMath::Max(FMath::RoundToInt(InValue), 1);
	}
	...
	{
		Yaw = FMath::Clamp(FMath::RoundToInt(InValue), -90, 90);
	}
	...
	{
		PerRow = FMath::Max(FMath::RoundToInt(InValue), 1);
	}
